...
for (i in 1:(dim(summerArray)[3]/3)){ #Dividing by 3 gives us the number of summer period (43). One less than the years we have (44) as summer spans two years each year.
  ...
  Summer95thHolder[, , i] = apply(subset, MARGIN = c(1,2), FUN = "quantile", na.rm = TRUE, probs = c(0.95)) #Calc se and store in ith layer
...
fishCounts = fishRaw %>% mutate(year = as.Date(survey_date) %>% format("%Y")) %>% 
  dplyr::select(site_code, site_name, location, year, survey_date, reporting_name, total) %>% 
  group_by(site_name, site_code, survey_date, year, location, reporting_name) %>% 
...
for (j in 1:length(variableList)){
  ...
  ToInterpolate = readRDS(filesList[j]) %>% dplyr::select(c(longitude, latitude, any_of(focalYears))) %>%  #Read in jth file and drop any columns we dont have animal data for
  st_as_sf(coords = c("longitude", "latitude"), crs = "EPSG:4326") %>% 
...
InsideCoastline = readRDS(filesList[1]) %>% dplyr::select(c(longitude, latitude, any_of(focalYears))) %>%  #Read in jth file and drop any columns we dont have animal data for
  st_as_sf(coords = c("longitude", "latitude"), crs = "EPSG:4326") %>% 
    st_transform(crs = "EPSG:7899") %>% 
    vect() %>% 
...
for (j in 1:length(variableList)){
  ...
  #Point count is still super high for the time we have to interpolate. Let's subset with a spatially balanced approach to 30%.
...
#Note we only have wave data up until 2020 so lets drop any fish data beyond this
fishDataSf = fishDataSf %>% filter(year %in% seq(1998, 2020))

#And repeat for invert data
invertDataSf = invertDataSf %>% filter(year %in% seq(1998, 2020))
...
for (i in years){
  ...
  setwd("Z:/GIS-Res/Projects/ParksVic_Hmsc/EnvironmentalLayers/AggregatedData/Rasters")
  stack1 = rast(
    list.files() %>% str_subset(pattern = as.character(i))) #Get wave and temp rasters
  ...
  names(stack)[23] = "Depth" #Rename to something simpler
  ...
  #Append to relevant year of fish data. i-2009 converts our active year to 1, 2, 3 etc  
  fishlist[[i-1997]] = fishDataSf %>% filter(year == i) %>% 
    vect() %>% 
    terra::extract(y = .,
                   x = stack,
                   bind = TRUE) %>% 
    ...
    st_drop_geometry() %>% 
    dplyr::select(-c(X,)) %>% 
    dplyr::select(1,2,3,4,5,263,264,265,266,277,278,267:276,253:262,6:253) #Reorder columns to something logical
  
  #Append to relevant year of invert data. i-2009 converts our active year to 1, 2, 3 etc  
  invertlist[[i-1997]] = invertDataSf %>% filter(year == i) %>% 
    vect() %>% 
    terra::extract(y = .,
                   x = stack,
                   bind = TRUE) %>% 
    ...
    st_drop_geometry() %>% 
    dplyr::select(-c(X,)) %>% 
    dplyr::select(1,2,3,4,5,268,269,270,271,282,283,272:281,258:267,6:258) #Reorder columns to something logical
...
#After some meetings 12.6.2024 we now only model data from 2002 onwards.
fishData = fishData[fishData$year %in% seq(2002, 2020, 1),]
invertData = invertData[invertData$year %in% seq(2002, 2020, 1),]
