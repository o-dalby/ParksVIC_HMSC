...
fishMaster = full_join(fishLatLon, fishCounts)
invertMaster = full_join(invertLatLon, invertCounts)
...
#Import same raster to use as template
templateRast = rast("Z:/GIS-Res/Projects/ParksVic_Hmsc/EnvironmentalLayers/Bathymetry/VICBathy50mMean.tif")
...
setwd("Z:/GIS-Res/Projects/ParksVic_Hmsc/EnvironmentalLayers/AggregatedData/Rasters")
imcraRast =  st_read("Z:/GIS-Res/Projects/ParksVic_Hmsc/EnvironmentalLayers/CBiCs/Integrated_Marine_and_Coastal_Regionalisation_of_Australia_(IMCRA)_v4.0_-_Provincial_Bioregions.shp") %>% 
  st_transform(crs = "EPSG:7899") %>%
  vect() %>% 
...
fishDataSf = terra::extract(x = imcraRast,
                      y = fishDataSf %>% vect(),
                      bind = TRUE) %>%
  st_as_sf()

invertDataSf = terra::extract(x = imcraRast,
                      y = invertDataSf %>% vect(),
                      bind = TRUE) %>%
  st_as_sf()
...
setwd("Z:/GIS-Res/Projects/ParksVic_Hmsc/EnvironmentalLayers/AggregatedData/Rasters")
mpaStatusRast = st_read("Z:/GIS-Res/Projects/ParksVic_Hmsc/EnvironmentalLayers/CBiCs/VEAC_CBiCS_classification_VIC.shp") %>% 
  st_transform(crs = "EPSG:7899") %>%  
  vect() %>% 
...
#Add column to dataframes
fishDataSf = terra::extract(x = mpaStatusRast,
                      y = fishDataSf %>% vect(),
                      bind = TRUE) %>%
  st_as_sf()

invertDataSf = terra::extract(x = mpaStatusRast,
                      y = invertDataSf %>% vect(),
                      bind = TRUE) %>%
  st_as_sf()
...
names(protectRast) = "MPA Protection Status"

setwd("Z:/GIS-Res/Projects/ParksVic_Hmsc/EnvironmentalLayers/AggregatedData/Rasters")
writeRaster(protectRast, "mpaProtectionRast.tif", overwrite = TRUE) #Export

#Add to sf dfs
fishDataSf = terra::extract(x = protectRast,
                      y = fishDataSf %>% vect(),
                      bind = TRUE) %>%
  st_as_sf()

invertDataSf = terra::extract(x = protectRast,
                      y = invertDataSf %>% vect(),
                      bind = TRUE) %>%
  st_as_sf()
...
That's the MPA stuff sorted...for now. Let's move on to the simpler albeit fiddly task of extracting each year of temperature and wave data and linking it to each fish and invert community.
