...
cor1 = fishData[,13:32] %>% cor(method = "pearson")
...
cor1 = fishData[,c(13:18, 20:21, 25, 27, 31, 32)] %>% cor(method = "pearson")
corrplot(cor1, method = "number", type = "lower", diag = FALSE)

#Terrains are better but the waves still trip us up. Repeat for just seasonal mean for temp and seasonal 95th for waves.
cor1 = fishData[,c(13:18, 20:21, 25, 31)] %>% cor(method = "pearson")
corrplot(cor1, method = "number", type = "lower", diag = FALSE)

#Lets run with that.
fishData = fishData[,c(1:12, 13:18, 20:21, 25, 31, 33:279)]

#Repeat for inverts.....
#Check intercor of all environmental predictors. Looking for Pearsons <0.7.
cor1 = invertData[,13:32] %>% cor(method = "pearson")
corrplot(cor1, method = "number", type = "lower", diag = FALSE)

#adjSD doesnt play well with vrm or qslope. The latter two dont play together either. All wave measures are >0.7. Temp doesn't look much better.
#Try again with just seasonal measure and SD for wave and temp. And drop Qslope and vrm. Mean for temp as longer times are needed to impact physiological stress but waves get 95th as a single big wave might do damage.
cor1 = invertData[,c(13:18, 20:21, 25, 27, 31, 32)] %>% cor(method = "pearson")
corrplot(cor1, method = "number", type = "lower", diag = FALSE)

#Terrains are better but the waves still trip us up. Repeat for just seasonal mean for temp and seasonal 95th for waves.
cor1 = invertData[,c(13:18, 20:21, 25, 31)] %>% cor(method = "pearson")
corrplot(cor1, method = "number", type = "lower", diag = FALSE)
...
invertData = invertData[,c(1:12, 13:18, 20:21, 25, 31, 33:284)]
...
```{r}
fishX = fishData[,c(3,4,8,13:22)]
fishX$year = as.integer(fishX$year) #Set year back to int

invertX = invertData[,c(3,4,8,13:22)]
...
```{r}
fishY = (fishData[,23:269])
invertY = (invertData[,23:273])
...
length(rareFish) #189 rare fish

rareInvert <- which(colSums(invertY>0)<49) #49 is ~5% of samples
length(rareInvert) #205 rare inverts
...
```{r}
fishS = fishData[,c(1,3,7,8,9,10,11,12)] #Grab site name, sampling, year, imcra bioregion, MPA protection status (protected or not?), the MPA name the points are associated with, and the broader geographic region as study design components. We may not use all of these in the model but at least they're there. Also grab lat longs

invertS = invertData[, c(1,3,7,8,9,10,11,12)] #As above
...
rL.fish.nngp <- HmscRandomLevel(sData = xy, longlat = FALSE, sMethod = 'NNGP',nNeighbours = 10)
rL.fish.nngp <- setPriors(rL.fish.nngp,nfMin=1,nfMax=1)
...
rL.invert.nngp <- HmscRandomLevel(sData = xy, longlat = FALSE, sMethod = 'NNGP',nNeighbours = 10)
rL.invert.nngp <- setPriors(rL.invert.nngp,nfMin=1,nfMax=1)
...
###### UPDATE BELOW AT SOME POINTS TOMORROW!!!!!!!!!! ####
