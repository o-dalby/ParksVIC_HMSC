...
fishTaxa = fishTaxa[str_c(fishTaxa$genus, fishTaxa$species, sep = ".") %in% names(fishY),]

str_c(fishTaxa$genus, fishTaxa$species, sep = ".") %>% table() #Check that we have only one entry per species. Got two for Pseudognoiistius nigripes.
fishTaxa[str_c(fishTaxa$genus, fishTaxa$species, sep = ".") == "Pseudogoniistius.nigripes",] #Shows we have two entries One with erroneous Family
fishTaxa = fishTaxa[!(str_c(fishTaxa$genus, fishTaxa$species, sep = ".") == "Pseudogoniistius.nigripes" & fishTaxa$family == "Cheilodactylidae"),]

str_c(fishTaxa$genus, fishTaxa$species, sep = ".") %in% names(fishY) %>% table() #Final check that all species names in tree match fishY. 58 TRUEs
...
invertTaxa = invertTaxa[str_c(invertTaxa$genus, invertTaxa$species, sep = ".") %in% names(invertY),]

str_c(invertTaxa$genus, invertTaxa$species, sep = ".") %>% table() #Check that we have only one entry per species. Got two for Cenolia.trichoptera and two Paguroidea spp.
invertTaxa[str_c(invertTaxa$genus, invertTaxa$species, sep = ".") == "Cenolia.trichoptera",] #Shows we have two entries one with erroneous order
invertTaxa = invertTaxa[!(str_c(invertTaxa$genus, invertTaxa$species, sep = ".") == "Cenolia.trichoptera" & invertTaxa$order == "Articulata"),]
invertTaxa[str_c(invertTaxa$genus, invertTaxa$species, sep = ".") == "Paguroidea.spp.",] #Shows we have two entries one with no family data
invertTaxa = invertTaxa[!(str_c(invertTaxa$genus, invertTaxa$species, sep = ".") == "Paguroidea.spp." & is.na(invertTaxa$family) == TRUE),]

str_c(invertTaxa$genus, invertTaxa$species, sep = ".") %in% names(invertY) %>% table() #Final check that all species names in tree match invertY. 46 TRUEs
...
Finally we can define our models. CURRENTLY DOES NOT WANT TO READ SPECIES NAMES FOR PHYLO TREE
